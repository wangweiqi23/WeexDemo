...
import com.eastmoney.live.ui.refreshlayout.LionRefreshDrawable;
...
public abstract class BaseBounceView<T extends View> extends FrameLayout {
    ...
    public BaseBounceView(Context context, boolean isSwipRefresh) {
        this(context, null, OrientationHelper.VERTICAL, isSwipRefresh);
    }
    ...
    public BaseBounceView(Context context, AttributeSet attrs, int orientataion) {
        this(context, attrs, orientataion, true);
    }

    public BaseBounceView(Context context, AttributeSet attrs, int orientataion, boolean
            isSwipRefresh) {
        super(context, attrs);
        mOrientation = orientataion;
        mIsSwipRefresh = isSwipRefresh;
        init(context);
    }
...
//            swipeLayout.finishPullLoad();
    ...
    private void createBounceView(Context context) {
        if (mIsSwipRefresh) {
            ...
            mSwipeLayout.setLayoutParams(new FrameLayout.LayoutParams(FrameLayout.LayoutParams
                    .MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT));
            innerView = setInnerView(context);
            if (innerView != null) {
                mSwipeLayout.addView(innerView, new FrameLayout.LayoutParams(FrameLayout
                        .LayoutParams
                        .MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT));
                mSwipeLayout.setEnabled(false);
                addView(mSwipeLayout, LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
            }
        } else {
            mRefreshLayout = new PullRefreshLayout(context);
            mRefreshLayout.setRefreshDrawable(new LionRefreshDrawable(getContext(),
                    mRefreshLayout));
            mRefreshLayout.setLayoutParams(new FrameLayout.LayoutParams(FrameLayout.LayoutParams
                    .MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT));
            innerView = setInnerView(context);
            if (innerView != null) {
                mRefreshLayout.addView(innerView, new FrameLayout.LayoutParams(FrameLayout
                        .LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT));
                mRefreshLayout.setEnabled(false);
                addView(mRefreshLayout, LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
            }
        }
    }
    ...
    public void setHeaderView(WXComponent refresh) {
        ...
        if (refresh != null && mSwipeLayout != null) {
            String colorStr = (String) refresh.getDomObject().getStyles().get(Constants.Name
                    .BACKGROUND_COLOR);
            ...
            if (bgColor != null) {
                if (!TextUtils.isEmpty(bgColor)) {
                    ...
                    if (!(colorInt == Color.TRANSPARENT)) {
                        mSwipeLayout.setColorSchemeColors(colorInt);
                    } else {
                        mSwipeLayout.setBackgroundColor(Color.BLUE);
                    }
                }
            }
        }
...
//
//                String colorStr = (String) loading.getDomObject().getStyles().get(Constants
...
//            swipeLayout.setPullLoadEnable(enable);
    ...
    public boolean isRefreshing() {
        boolean res = false;
        if (mSwipeLayout != null && mSwipeLayout.isEnabled()) {
            res = mSwipeLayout.isRefreshing();
        } else if (mRefreshLayout != null && mRefreshLayout.isEnabled()) {
            res = mRefreshLayout.isRefreshing();
        }
        return res;
    }
