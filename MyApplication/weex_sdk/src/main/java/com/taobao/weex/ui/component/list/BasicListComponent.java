...
public abstract class BasicListComponent<T extends ViewGroup & ListComponentView> extends WXVContainer<T> implements
  ...
  private WXSwipeLayout.WXOnLoadingListener mLoadingListener;
    ...
    bounceRecyclerView.getInnerView().addOnScrollListener(new RecyclerView.OnScrollListener() {
      ...
      public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
        ...
        if (newState == RecyclerView.SCROLL_STATE_IDLE) {
          ...
          if (mLoadingListener != null && (mChildren != null && mChildren.size()>  mMinLoadCount)
  ...
  protected boolean setProperty(String key, Object param) {
    switch (key) {
      ...
      case Constants.Name.SCROLLABLE:
        ...
        return true;
      case Constants.Name.MIN_LOAD_COUNT:
        int count = WXUtils.getInt(param);
        setMinLoadCount(count);
        return true;
    }
  ...
  @WXComponentProp(name = Constants.Name.MIN_LOAD_COUNT)
  public void setMinLoadCount(int count) {
    this.mMinLoadCount = count;
  }
...
//        || (component instanceof WXLoading)
      ...
      if (WXEnvironment.isApkDebugable()) {
        WXLogUtils.d(TAG, "Bind WXRefresh " + holder);
      }
      return;
    }
...
//              holder.getComponent().refreshData(component);
    } else if (holder.getComponent() != null && holder.getComponent() instanceof WXLoading) {
      //处理loadview
      holder.getComponent().bindData(component);
    }
  ...
  public boolean isSwipRefreshLayout() {
    String type = null;
    if (getDomObject() != null) {
      type = WXUtils.getString(getDomObject().getAttrs().get(Constants.Name.REFRESH_TYPE), SWIPE);
    }
    return SWIPE.equalsIgnoreCase(type);
  }

  public void setLoadingListener(WXSwipeLayout.WXOnLoadingListener mLoadingListener) {
