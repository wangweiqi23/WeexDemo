...
public class WXCircleViewPager extends ViewPager implements WXGestureObservable {
  ...
  private static final int MESSAGE_SCROLL_ACTION = 1001;
  ...
  public void startAutoScroll() {
    ...
    mHandler.sendEmptyMessageDelayed(MESSAGE_SCROLL_ACTION, intervalTime);
  }
  ...
  private static class MyHandler extends Handler {
    private SoftReference<WXCircleViewPager> mRef;

    private MyHandler(WXCircleViewPager viewPager) {
      mRef = new SoftReference<>(viewPager);
    }

    @Override
    public void handleMessage(Message msg) {
      super.handleMessage(msg);
      if (msg.what == MESSAGE_SCROLL_ACTION) {
        WXCircleViewPager view = mRef.get();
        removeCallbacksAndMessages(null);
        if (view == null) {
          return;
        }
        WXLogUtils.d("[CircleViewPager] trigger auto play action ");
        view.superSetCurrentItem(view.superGetCurrentItem() + 1, true);
        sendEmptyMessageDelayed(MESSAGE_SCROLL_ACTION, intervalTime);
      }
