...
public class WXLoading extends WXBaseRefresh implements WXSwipeLayout.WXOnLoadingListener {
    ...
    private WXLoadingLayout mWXLoadingLayout;
    ...
    protected WXFrameLayout initComponentHostView(@NonNull Context context) {
        mWXLoadingLayout = new WXLoadingLayout(context);
        return mWXLoadingLayout;
    }
    ...
    public void onLoading() {
        if (mStatus == Constants.LoadStatus.NORMAL && getDomObject().getEvents().contains(Constants
                .Event.ONLOADING)) {
            mStatus = Constants.LoadStatus.LOADING;
    ...
    public void onPullingUp(float dy, int pullOutDistance, float viewHeight) {
        if (getDomObject().getEvents() != null && getDomObject().getEvents().contains(Constants
                .Event.ONPULLING_UP)) {
    ...
    public void setDisplay(String display) {
        if (!TextUtils.isEmpty(display)) {
            if (getParent() instanceof WXListComponent) {
                updateListLoadView(display);
            } else if (getParent() instanceof WXScroller) {
                if (display.equals(HIDE)) {
                    if (((BaseBounceView) getParent().getHostView()).isRefreshing()) {
                        loadComplete();
                    }
                }
            }
    ...
    private void updateListLoadView(String display) {
        if (display.equals(SHOW)) {
            mStatus = Constants.LoadStatus.NORMAL;
            mWXLoadingLayout.showLoading();
        } else if (display.equals(END)) {
            mStatus = Constants.LoadStatus.END;
            mWXLoadingLayout.showEnd();
        } else if (display.equals(HIDE)) {
            mStatus = Constants.LoadStatus.NORMAL;
            mWXLoadingLayout.showLoading();
        } else if (display.equals(ERROR)) {
            mStatus = Constants.LoadStatus.NORMAL;
            mWXLoadingLayout.showError();
        }
        //不特殊处理其它事件
    }

    private void loadComplete() {
        if (getParent() != null && getParent().getHostView() != null) {
