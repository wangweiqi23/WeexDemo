        ...
        WXSDKManager.getInstance().postOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                ImageRequest request = ImageRequestBuilder
                        ...
                        .setResizeOptions(getResizeOptions(view.getLayoutParams().width, view
                                .getLayoutParams().height, quality))
...
//                            LogUtil.d(TAG,String.format("Final image received! Size %d x %d Quality level %d, good enough: %s, full quality: %s",imageInfo.getWidth(),
//                                    imageInfo.getHeight(),
...
//                            LogUtil.e(TAG,throwable);
                    ...
                    DraweeView imgeView = (DraweeView)view;
                    DraweeController controller = Fresco.newDraweeControllerBuilder()
                        .setAutoPlayAnimations(false)
                        .setOldController(((DraweeView)view).getController())
                        .setControllerListener(controllerListener)
                        .setUri(uri)
                        .setImageRequest(request)
                        .build();

                    imgeView.setController(controller);
    ...
    /**
     * weex底层代码bug 这里暂时简单处理
     * weex 默认image数据 low类型
     * @param width
     * @param height
     * @param quality
     * @return
     */
    private ResizeOptions getResizeOptions(int width, int height, WXImageQuality quality) {
        if (quality == WXImageQuality.LOW) {
            width = width / 2;
            height = height / 2;
        } else if (quality == WXImageQuality.NORMAL) {
            width = (int) (width * 0.8);
